...
function TerminalAuthCard({
  ...
  const fullCommand = command ? buildTerminalCommand(command, connectedArgs, method.args ?? [], method.env) : null;
...
export function AgentInfoModal({
  ...
  return (
    <Modal
      onClose={onClose}
      className="max-h-[80vh] w-[500px] overflow-y-auto native-scroll rounded-lg border border-border bg-background p-5 shadow-xl select-text"
    >
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {capabilities.promptCapabilities &&
              Object.entries(capabilities.promptCapabilities).map(([key, val]) => (
                <CapBadge key={`prompt-${key}`} label={key} enabled={typeof val === 'boolean' ? val : Boolean(val)} />
              ))}
            {capabilities.mcpCapabilities &&
              Object.entries(capabilities.mcpCapabilities).map(([key, val]) => (
                <CapBadge
                  key={`mcp-${key}`}
                  label={`mcp:${key}`}
                  enabled={typeof val === 'boolean' ? val : Boolean(val)}
                />
              ))}
          </div>
        </div>
