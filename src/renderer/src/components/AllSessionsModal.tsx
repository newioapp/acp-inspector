...
export function AllSessionsModal({ onClose }: { readonly onClose: () => void }): React.JSX.Element {
  ...
  return (
    <Modal
      onClose={onClose}
      className="w-[480px] max-h-[70vh] flex flex-col rounded-lg border border-border bg-background shadow-xl"
    >
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <h2 className="text-sm font-semibold">
          All Sessions ({String(filtered.length)}
          {filter ? ` / ${String(sessions.length)}` : ''})
        </h2>
          ...
          >
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className="truncate text-xs font-medium text-foreground">{s.title ?? 'Untitled session'}</span>
                {!s.loaded && (
              ...
              <div className="mt-1 space-y-0.5 text-[10px] text-muted-foreground">
                <div className="font-mono truncate" title={s.sessionId}>
                  {s.sessionId}
                </div>
                {s.cwd && (
                  <div className="truncate" title={s.cwd}>
